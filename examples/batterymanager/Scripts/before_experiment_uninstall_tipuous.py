...
"""``before_experiment`` hook: chain device-state controls + BATTERY_STATS auto-grant + Tipuous uninstall.

The original responsibility of this hook (uninstall the previous Tipuous install
...
Chained hooks (in order):
  ...
  3. Original Tipuous uninstall: removes any prior install so the experiment's
...
def main(device, *args, **kwargs):
    try:
        ...
        try:
            sys.stderr.write(
                "before_experiment_uninstall_tipuous: device-state controls chain "
    ...
    try:
        ...
        try:
            sys.stderr.write(
                "before_experiment_uninstall_tipuous: BATTERY_STATS auto-grant chain "
    ...
    try:
        ...
        try:
            sys.stderr.write(
                "before_experiment_uninstall_tipuous: uninstall step failed "
