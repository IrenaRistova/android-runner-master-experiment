...
"""``before_experiment`` hook: chain device-state controls + BATTERY_STATS auto-grant + LinkHub uninstall.

The original responsibility of this hook (uninstall the previous LinkHub install
...
Chained hooks (in order):
  ...
  3. Original LinkHub uninstall: removes any prior install so the experiment's
...
def main(device, *args, **kwargs):
    try:
        ...
        try:
            sys.stderr.write(
                "before_experiment_uninstall_linkhub: device-state controls chain "
    ...
    try:
        ...
        try:
            sys.stderr.write(
                "before_experiment_uninstall_linkhub: BATTERY_STATS auto-grant chain "
    ...
    try:
        ...
        try:
            sys.stderr.write(
                "before_experiment_uninstall_linkhub: uninstall step failed "
